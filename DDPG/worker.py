...
class Worker :
	def __init__(self,index,model,env,memory,preprocess=T.ToTensor(),path=None,frompath=None,num_episodes=1000,HER={'use_her':True,'k':4,'strategy':'future','singlegoal':False},use_cuda=True,rendering=False) :
		...
		self.workerfn = lambda: trainIN( index=self.index,
										model=self.model,
										env=self.env,
										memory=self.memory,
										logger=self.sl,
			...
			for t in count() :
				...
				if done :
					...
					for it in range(nbrTrain) :
						since = time.time()
						retloss = model.optimize()
						if retloss is not None :
							critic_loss,actor_loss = retloss
...
def trainIN(index,model,env,memory,logger=None,preprocess=T.ToTensor(),path=None,frompath=None,num_episodes=1000,HER={'use_her':True,'k':4,'strategy':'future','singlegoal':False},use_cuda=True,rendering=False): 
	try :
		episode_durations = []
		episode_reward = []
		episode_qsa = []
