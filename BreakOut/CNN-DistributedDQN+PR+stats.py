	...
	try :
		...
		while len(memory) < MIN_MEMORY :
			...
			for t in count() :
				...
				episode_buffer.append( EXP(state,action,next_state,torch.ones(1)*reward,done) )
...
def train(model,env,memory,optimizer,logger=None,preprocess=T.ToTensor(),path=None,frompath=None,num_episodes=1000,epsend=0.05,epsstart=0.9,epsdecay=200): 
	try :
		...
		accumulateMemory(memory,env,model,preprocess,epsstart=0.99,epsend=0.9,epsdecay=200)
...
def main():
	...
	model_path = './'+env+'::CNN+DQN+PR-alpha'+str(alphaPER)+'-w'+str(num_worker)+'-lr'+str(lr)+'-b'+str(BATCH_SIZE)+'-m'+str(memoryCapacity)+'/'
	#mkdir :
