...
class Worker :
	...
	def trainOUT(self,index,model,env,memory,optimizers,logger=None,preprocess=T.ToTensor(),path=None,frompath=None,num_episodes=1000,HER={'use_her':True,'k':4,'strategy':'future','singlegoal':False},use_cuda=True,rendering=False): 
		try :
			episode_durations = []
			episode_reward = []
			episode_qsa = []
			...
			nbr_ep_train = self.nbr_ep_per_train
			...
			for i in range(num_episodes) :
				...
				for t in count() :
					...
					if done or t> MAX_STEP:		
						...
						meanaction = np.mean(action_buffer)
						sigmaaction = np.std(action_buffer)
						action_buffer_hd = np.array(action_buffer).reshape((-1))
						
						if usehd : hd.append(np.array(action_buffer_hd) )

						#log = 'Episode duration : {}'.format(t+1) +'---' +' Action : mu:{:.4f} sig:{:.4f} // Reward : {} // Mean C/A Losses : {:.4f}/{:.4f} // Mean/MaxQsa : {:.4f}/{:.4f} // Mean Actor Grad : {:.8f}'.format(meanaction,sigmaaction,cumul_reward,meancloss,meanaloss,meanqsa,maxqsa,meanactorgrad) +'---'+' {}Hz'.format(meanfreq)
						log = 'Episode duration : {}'.format(t+1) +'---' +' Action : mu:{:.4f} sig:{:.4f} // Reward : {} '.format(meanaction,sigmaaction,cumul_reward)
						#// Mean C/A Losses : {:.4f}/{:.4f} // 
						log += 'Mean/MaxQsa : {:.4f}/{:.4f} '.format(meanqsa,maxqsa)
						#// Mean Actor Grad : {:.8f}'.format(meanaction,sigmaaction,cumul_reward,meancloss,meanaloss,meanqsa,maxqsa,meanactorgrad) 
						log += '---'+' {}Hz'.format(meanfreq)
						...
						if logger is not None :
							#new = {'episodes':[i],'duration':[t+1],'reward':[cumul_reward],'mean frequency':[meanfreq],'critic loss':[meancloss],'actor loss':[meanaloss],'max qsa':[maxqsa],'mean qsa':[meanqsa],'mean action':[meanaction]}
							new = {'episodes':[i],'duration':[t+1],'reward':[cumul_reward],'mean frequency':[meanfreq],'max qsa':[maxqsa],'mean qsa':[meanqsa],'mean action':[meanaction]}
							logger.append(new)
