...
def optimize_model(model,memory,optimizer) :
	...
	#UPDATE THE PR :
	for (idx, new_error) in zip(batch.idx,loss.cpu().data.numpy()) :
...
def main():
	...
	model_path = './CNN+DQN+PR-alpha'+str(alphaPER)+'-lr'+str(lr)+'-b'+str(BATCH_SIZE)+'-m'+str(memoryCapacity)+'/'
	#mkdir :
	if not os.path.exists(model_path) :
		os.mkdir(model_path)
	path=model_path+'breakout.save'
	frompath = None

	if path in os.listdir(model_path) :
		frompath = path
